...
const Select2 = (props)=>{
  ...
  function handleChooseOptionValue(item){
    if(onChange){
      onChange({target: { name: name, value: item[optionsName.key] }})
    }
  ...
  return (
    <div className={classes}>
      ...
      <label className={["select_label", isInputError ? "select_label_error" : ""].join(" ")} htmlFor={name}>{label}</label>
      ...
      <div className="input_wrapper">
        ...
        <div style={chevonStyle} onClick={handleShowSuggetion} className="switch_select">
          <i className={['fa fa-chevron-up', isShowSuggetion ? 'chevron_up' : 'chevron_down'].join(" ")} aria-hidden="true"></i>
        </div>
